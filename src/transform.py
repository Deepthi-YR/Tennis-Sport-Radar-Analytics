...
def _value(item: dict, *names: str, default=None):
    return next(
        (item[n] for n in names if item.get(n) is not None),
        default
    )
...
def competitions_payload(payload: dict):
    ...
    for item in payload.get("competitions", []):
        ...
        competitions.append({
            "competition_id": _value(item, "id"),
            "competition_name": _value(
                item,
                "name",
                default="Unknown"
            ),
        ...
        for venue in item.get("venues", []):
            ...
            venues.append({
                ...
                "country_name": _value(
                    country,
                    "name",
                    default=venue.get("country_name")
                ),
                "country_code": _value(
                    country,
                    "code",
                    default=venue.get("country_code")
                ),
...
def rankings_payload(payload: dict):
    ...
    for league_index, league in enumerate(
        ...
        # The endpoint contains ranking groups
        # for ATP and WTA.
        for item in league.get(
            ...
            ranking_group = (
                league.get("name")
                or league.get("gender")
                or str(league_index + 1)
            )

            rankings.append({
                "rank_id": (
                    (league_index + 1) * 10000
                    + item.get(
                        "rank",
                        len(rankings) + 1
                    )
